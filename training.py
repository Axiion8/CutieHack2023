...
for image_class in os.listdir(DIRECTORY):
    ...
    try:
        ...
        #cv2.imread(os.path.join('Dataset', 'Panda', 'Datasets/Panda/images82.jpg'))
...
data = tf.keras.utils.image_dataset_from_directory('Datasets', batch_size = 32) #Builds image data set, meaning you need not build classes or labels. 
...
for idx, img in enumerate(batch[0][:4]): #Should display 2 pictures according to dataset
...
data = data.map(lambda x,y: (x / 255,y)) #data.map allows you to preform the transformation in pipline of dividing by 255.
...
#data.as_numpy_iterator().next() #Gives access to iterator and grabs next batch. Note you have to shuffle or else data will not change
 ...
 # .next() iterates and gives new batch
...
#Fit our model:
hist = model.fit(train, epochs=5, validation_data=val, callbacks=[tensorboard_callback]) #Size of training data must be larger than batch size
...
img = cv2.imread('arcticfox.jpg') #Read NEW image that model hs not scene #upload picture to directory here and the machine learning animal will determine which animal it is
plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)) #converts to normal picture color
plt.show()
...
yPredict = model.predict(np.expand_dims(resize/255,0))
...
model.save(os.path.join('models', 'animals.h5')) #saves new models each run

new_model = load_model(os.path.join('models','animals.h5')) #loads model 
